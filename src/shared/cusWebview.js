...
const CusWebview = (props) => {
  ...
  const onWebViewMessage = (datas) => {
    ...
    if (jsonData.type == 'login_check') {
      ...
      setFooterUse(false);
      setIsLoading(true);
    } else if (jsonData.type == 'downfile') {
      saveToGallery(jsonData.data, jsonData.fileName);
    } else if (jsonData.type == 'loadingOn') {
      setIsLoading(true);
  ...
  //닫기가 나와야될 페이지
  const closeArray = ['/word/word.php'];
  const homeArray = ['/word/word.php'];
    ...
    <View style={{flex: 1}}>
      ...
      {isConnected == true ? (
        <View style={{flex: 1}}>
          ...
          <WebView
            ref={webViews}
            userAgent={userAgent + ' -- RNApp -- ' + Platform.OS}
            source={{uri: urls}}
            onLoadEnd={(webViews) => {
